// Edge Impulse Fall Detection (Foot)
...
// Import ei-falldetection_foot-arduino-1.0.2
#include <Arduino.h>
#include <Wire.h>
#include <FallDetection_Foot_inferencing.h>
...
// Foot-tuned thresholds: faster EMA to capture impacts at the shoe and quicker post-impact evaluation.
static constexpr float FALL_EMA_ALPHA = 0.45f;  // exponential smoothing for p(FALL)
...
static void poll_help_prompt_inputs() {
  ...
  if (help_touch_ready && help_touch_panel) {
    ...
    // Debug: log detections while estamos en la pantalla de ayuda.
...
static void update_help_prompt_state() {
  ...
  if (help_answer_yes || timeout_elapsed) {
    const bool from_timeout = (!help_answer_yes && timeout_elapsed);
    resolve_help_prompt(true,
      from_timeout ?
        "[EI] No response after 5 s, requesting help automatically" :
        "[EI] User confirms help is needed");
  } else if (help_answer_no) {
